...
- Agent 调度用 Seedmux 的 `seedmux-team` skill 和个人外置入口 `~/.local/bin/smx-team`（复用官方桥接）；不在本仓库复制官方 skill。所有子代理和审查者必须由 Seedmux 创建；禁止 Butler UI 自建子会话、Worker 池或另一个主控。Master 职责属于 Seedmux 中的当前主控 pane。
