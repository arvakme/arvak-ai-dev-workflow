...
import { cleanupButlerUIModules, copyButlerUISource, BUTLER_UI_DIR, loadButlerUIModule } from "./loader.ts";
...
test("portable loader copies runtime sources without repository metadata or development docs", async () => {
	const directory = await mkdtemp(join(tmpdir(), "butler-ui-copy-"));
	try {
		await copyButlerUISource(directory);
...
test("missing runtime config disables optional behavior and warns on each session_start", async () => {
	const { default: registerButlerUI } = await loadButlerUIModule("index.ts", { configJsonc: null });
	...
	(registerButlerUI as (pi: unknown) => void)(pi);
	...
	expect(warnings).toEqual([
		"Butler UI 配置有问题：config.jsonc 不存在，已关闭可选功能",
		"Butler UI 配置有问题：config.jsonc 不存在，已关闭可选功能",
	]);
});
...
test("runtime config enables only its declared behavior", async () => {
	...
	const { default: registerButlerUI } = await loadButlerUIModule("index.ts", { configJsonc });
	const commands: string[] = [];
	const shortcuts: string[] = [];
	(registerButlerUI as (pi: unknown) => void)({
		registerCommand: (name: string) => commands.push(name),
		registerShortcut: (key: string) => shortcuts.push(key),
...
test("preset 只认模型原子，旧的三字段写法被拒", async () => {
	const { loadConfig } = await loadButlerUIModule("config.ts", {
...
test("公共配置模板可解析并启用完整推荐工作流", async () => {
	const configJsonc = await readFile(join(BUTLER_UI_DIR, "config.example.jsonc"), "utf8");
	const { loadConfig } = await loadButlerUIModule("config.ts", { configJsonc });
	const loaded = (loadConfig as () => { config: any; problems: string[] })();
