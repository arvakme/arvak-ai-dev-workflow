...
import { cleanupButlerUIModules, loadButlerUIModule } from "./loader.js";
...
const load = async (): Promise<Module> =>
	(cached ??= (await loadButlerUIModule("session/herdr-display.js")) as unknown as Module);
...
async function herdrStub(failFirst = false) {
	const directory = await mkdtemp(join(tmpdir(), "butler-ui-herdr-"));
