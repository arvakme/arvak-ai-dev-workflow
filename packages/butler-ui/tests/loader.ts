...
export const BUTLER_UI_DIR = dirname(dirname(fileURLToPath(import.meta.url)));
const SOURCE_DIR = BUTLER_UI_DIR;
...
export async function copyButlerUISource(destination: string): Promise<void> {
 ...
 */
export async function prepareButlerUIFixture(
	entry: string,
	options: {
	...
	const directory = await mkdtemp(join(tmpdir(), "butler-ui-test-"));
...
export async function loadButlerUIModule(entry: string, options: Parameters<typeof prepareButlerUIFixture>[1] = {}): Promise<Record<string, unknown>> {
	const { directory } = await prepareButlerUIFixture(entry, options);
	return import(`${pathToFileURL(join(directory, entry)).href}?test=${Date.now()}`);
}

export async function cleanupButlerUIModules(): Promise<void> {
