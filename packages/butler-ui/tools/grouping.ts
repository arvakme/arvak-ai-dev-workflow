...
export const BUTLER_UI_TOOLS = new Set(["read", "bash", "edit", "write", "find", "grep", "ls"]);
...
function patchToolRows(): void {
	...
	const patched: RenderFunction = function (this: ToolRow, width: number): string[] {
		...
		const isButlerUITool = BUTLER_UI_TOOLS.has(toolName);
		const isExpanded = Boolean((this as any).expanded);
		const lines = isButlerUITool || isExpanded
