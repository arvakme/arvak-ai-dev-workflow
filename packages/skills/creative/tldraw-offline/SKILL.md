---
name: tldraw-offline
description: 用本机 tldraw Desktop 画或改流程图和画布。打开 .tldraw、摆方块、绑箭头、截图、保存。用户说流程图、tldraw、/tldraw-offline 时用。
---

# tldraw-offline

本机 tldraw Desktop 画布。默认当**流程图**：方块节点 + 绑定箭头，不是装饰线。应用没开就停，让用户打开 tldraw-offline。

## 流程图（默认路径）

1. 确认应用在跑：读 `~/Library/Application Support/tldraw/server.json` 的 port/token；请求失败就说应用没开。
2. 已有窗口就按文件名/聚焦窗口下手；没有就 `POST /api/docs/create` 新建 `.tldraw`（不要手写文件）。
3. 节点用 `geo` 方块 + `toRichText` 标签，从左到右或从上到下留空隙。
4. 每条有意义的连线用 `helpers.createArrowBetweenShapes(fromId, toId)`，禁止未绑定的裸箭头。
5. `helpers.getLints()`，有问题就修。本地文档 `await helpers.saveDoc()`，不要让用户 Cmd+S。
6. 需要时 `api.getScreenshot` 看一眼布局。报完 doc 名、形状 id、lint 结果就停。

斜杠命令：`/tldraw-offline [画什么]`。

# tldraw canvas operator

The desktop app exposes a local HTTP server that can list documents, inspect canvas state, capture screenshots, execute JavaScript against a live editor, and expose live script files for durable behavior.
...
A ready-made helper ships next to this SKILL.md as `tq`. Resolve that path to an absolute path (this skill directory), then `sh <skill-dir>/tq <METHOD> <path> [body]` — it re-reads the port and token from `server.json` itself on every call, so you never handle the token or the fresh-shell env problem. A body starting with `{` is sent as JSON; anything else as raw `text/plain`:

```bash
sh <skill-dir>/tq POST /api/search '{"code":"return await api.getDocs()"}'
sh <skill-dir>/tq POST /api/doc/DOC_ID/exec 'return editor.getCurrentPageShapes().length'
sh <skill-dir>/tq GET  /api/doc/DOC_ID/script-status
```

If `tq` is missing, fall back to raw `curl` with the `PORT`/`TOKEN` reads shown above. The raw-`curl` examples below stay in explicit form so each request is visible; translate any to `sh <skill-dir>/tq <METHOD> <path> [body]`.
...
Shown as raw `curl`; `sh <skill-dir>/tq <METHOD> <path> [body]` is the shorter equivalent that handles the port and token for you.
