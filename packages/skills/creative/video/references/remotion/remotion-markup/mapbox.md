---
name: maps-mapbox
description: 使用 Mapbox GL JS 和 Turf 制作确定性 Remotion 地图动画。当用户为动画路线、立交桥、地图标记、标签、相机移动或 Mapbox 样式选择 Mapbox 时使用。
metadata:
  tags: map, map animation, mapbox, turf, geojson, route animation
---

当用户想要 Mapbox 样式或更高保真度的地图视觉效果并具有 Mapbox 访问令牌时，请使用 Mapbox GL JS 在 Remotion 中渲染地图。使用 Turf 进行地理空间操作，例如大圆路线、距离、切片线和沿路线的位置。

如果用户没有 Mapbox 访问令牌或想要开源渲染器，请改用 [maplibre.md](maplibre.md)。

## 核心规则

- 对于地理空间工作，首选 `@turf/turf`。除非用户明确需要自定义非测地线效果，否则请勿手动滚动距离、大圆、路线切片或坐标插值。
- 使用 GeoJSON 源和 Mapbox 图层作为线条、标记和标签。避免使用 DOM `Marker` 元素，除非用户特别要求使用 HTML 标记。
- 禁用非确定性地图行为：`interactive: false`、`fadeDuration: 0`。
- 在地图加载和每帧地图更新方面使用 `delayRender()` / `continueRender()`。
- 在继续初始渲染之前，添加sources/layers，使用`jumpTo()`或`setFreeCameraOptions()`应用第0帧相机，然后等待`idle`。
- 不要添加`mapInstance.remove()`清理功能；它会干扰 Remotion 的渲染生命周期。
- 使用 Mapbox 样式 URL，例如 `mapbox://styles/mapbox/standard` 或用户提供的自定义样式。
- 不要安装`@types/mapbox-gl`； Mapbox GL JS 提供自己的类型。

Mapbox、Turf 和 GeoJSON 中的坐标为 `[longitude, latitude]`。
...
使用项目的包管理器安装 Mapbox GL JS 和 Turf。
...
在组件或应用程序级样式表中导入一次 Mapbox CSS：

```ts
import 'mapbox-gl/dist/mapbox-gl.css';
```

Mapbox 需要公共访问令牌。更喜欢将其作为输入属性传递或从捆绑的 Remotion 代码可用的环境变量中读取它。
...
- Turf 创建路线和标记。
- Turf 对路线进行切片以显示线条动画。
- Mapbox 使用GeoJSON 源和图层渲染路线。
- 相机使用 `jumpTo()` 以及动画中心、变焦、方位和俯仰。
...
对于大多数路线动画，请使用 `jumpTo()` 对 `center`、`zoom`、`bearing` 和 `pitch` 进行动画处理：
...
如果相机需要超前、滞后、缩小或放大，请将路线进度和相机进度分开。对于电影 3D 相机移动，仅当项目需要额外控制时才使用 Mapbox 的免费相机 API。
...
如果用户请求其他样式，请使用任何有效的 Mapbox 样式 URL。
