...
`index.ts` 按 `config.features` 注册独立功能。状态只归所属模块；`session/pet.ts` 通过不抢焦点的 overlay 渲染 Butler（零高度 widget 仅负责释放资源），`session/butler-frames.ts` 提供像素帧。只读取本会话生命周期，位置按 Pi custom entry 保存；不创建状态目录或桌面进程。
