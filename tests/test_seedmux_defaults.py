...
class DefaultsTests(unittest.TestCase):
    ...
    def test_preserves_other_settings_and_is_idempotent(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(seed['agents'], {'opencode_yolo': False})
            local = tomllib.loads((h / '.seedmux/config.local.toml').read_text())
            self.assertEqual(local['agents'], {a + '_yolo': True for a in m.AGENTS})
            # App regeneration does not replace personal overrides.
